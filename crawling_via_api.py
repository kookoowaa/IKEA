...
api_token = input('you api token is?: ')
api_token = '?api_token=' + api_token
api_secret = input('you api secret key is?: ')
api_secret = '&api_token_secret=' + api_secret
fromwhen = '&filter[field][0]=date_submitted&filter[operator][0]=>&filter[value][0]={t}'.format(t=dateofsurvey)
...
## transform data
tmp_dict = {'id':[], 'date':[], 'touchpoint':[], 'interaction_id':[], 'CSAT':[], 'resolution':[], 'first_contact':[], 'freetext':[]}
print('initiating....')
for cnt in range(nLoop):
    ...
    for survey in json_file['data']:
        ...
        try:
            tmp_dict['touchpoint'].append(survey['survey_data']['227']['answer'])
        except:
            tmp_dict['touchpoint'].append(None)
        try:
            tmp_dict['interaction_id'].append(survey['survey_data']['221']['answer'])
        except KeyError:
            tmp_dict['interaction_id'].append(None)
        try:
            for i in survey['survey_data']['77']['options']:
                tmp_dict['CSAT'].append(survey['survey_data']['77']['options'][i]['answer'])
        except KeyError:
            tmp_dict['CSAT'].append(None)
        try:
            tmp_dict['resolution'].append(survey['survey_data']['250']['answer'])
        except KeyError:
            tmp_dict['resolution'].append(None)
        try:
            tmp_dict['first_contact'].append(survey['survey_data']['251']['answer'])
        except KeyError:
...
final_df.to_csv('PulseCX({f}-{t}).csv'.format(f=dateofsurvey, t=date.today().isoformat()), encoding='utf-8-sig', index=False)
